...
| 全仓 TypeScript | 通过 |
| DB tests | 3/3 通过 |
| API tests | 87/87 通过（含 Demo card、ERP inventory/customer webhook 与 checkout promotion） |
| Worker tests | 4/4 通过 |
...
| Local multi-process staging E2E | 编译 API + Worker + ERP mock + Mailpit + 隔离 PostgreSQL：Demo Canada Post、Demo card、manual payment、order→ERP webhook→email 全链路通过 |
